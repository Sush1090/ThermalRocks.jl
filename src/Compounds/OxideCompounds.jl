...
struct SiO₂ <: OxideCompounds
    ...
    function SiO₂(;name = "QUARTZ",A = [8.145E+01 1.828E-03 -1.810E+05 -6.985E+02 5.406E-06;
                                        5.796E+01 9.330E-03 1.835E+0 0 0],MolarMass = 60.084,T_range = [(298,844),(844,1700)],PieceWise = true)
        #if name == "QUARTZ"
            if PieceWise ==false
                @assert size(A,1) == size(T_range,1) 
                new(name,A,MolarMass,T_range,PieceWise)
...
struct Fe₂O₃ <: OxideCompounds
    ...
    T_range::Vector{Tuple}
    PieceWise::Bool
    function Fe₂O₃(;name = "HEMATITE",A = [8.378E+02 -6.656E-01 4.989E+06 -1.075E+0 3.487E-04;
                                            -1.096E+03 2.727E-01 -1.024E+08 3.396E+0 0],MolarMass = 159.692,T_range = [(298,950),(950,1800)],PieceWise = true)
            @assert size(A,1) == size(T_range,1) 
            new(name,A,MolarMass,T_range,PieceWise)
    end
end
